...
function set_attributes(access_key_id, secret_signing_key, secret_crypto_access_key, host) {
  let server = (host) ? host : "https://api.cryptograpi.com"
...
class Credentials {
  constructor(access_key_id, secret_signing_key, secret_crypto_access_key, host) {
    this.access_key_id = (access_key_id) ? access_key_id : process.env.CRYPTOGRAPI_ACCESS_KEY_ID
    this.secret_signing_key = (secret_signing_key) ? secret_signing_key : process.env.CRYPTOGRAPI_SECRET_SIGNING_KEY
    this.secret_crypto_access_key = (secret_crypto_access_key) ? secret_crypto_access_key : process.env.CRYPTOGRAPI_SECRET_CRYPTO_ACCESS_KEY
    this.host = (host) : process.env.CRYPTOGRAPI_SERVER
    return (set_attributes(
      this.access_key_id,
      this.secret_signing_key,
...
class ConfigCredentials {
  constructor(config_file, profile) {
    if (!config_file) {
      config_file = require('os').homedir() + './cryptograpi/credentials'
      if (!fs.existsSync(config_file)) {
        config_file = require('os').homedir() + './cryptograpi/credentials.json'
      }
    }
    return (this.load_credentials(credentials_data, profile)) 
  }
  ...
  load_credentials(credentials_file, profile) {
    ...
    try {
      ...
      // ConfigParser requires file name, not data
